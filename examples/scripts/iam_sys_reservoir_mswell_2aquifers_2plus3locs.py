...
if __name__ == '__main__':
    ...
    for i, crds in enumerate(well_xys):
        ...
        aress[-1].add_par('injRate', min=0.1, max=10.0, value=1.0)
        aress[-1].add_par('logResPerm', min=-13.5, max=-12.5, value=-13.0)
        aress[-1].add_par('shale1Thickness', min=10.0, max=25.0, value=15.0)
        aress[-1].add_par('shale2Thickness', min=10.0, max=25.0, value=25.0)
        aress[-1].add_par('shale3Thickness', min=300.0, max=500.0, value=350.0)
        aress[-1].add_par('aquifer1Thickness', min=100.0, max=150.0, value=120.0)
        aress[-1].add_par('aquifer2Thickness', min=100.0, max=140.0, value=110.0)
        aress[-1].add_par('reservoirThickness', min=30.0, max=50.0, value=40.0)
        aress[-1].add_par('brineResSaturation', min=0.1, max=0.3, value=0.2)
...
#        aress[-1].add_obs('mass_CO2_reservoir')
        ...
        mss[-1].add_par_linked_to_par('shale1Thickness', aress[-1].pars['shale1Thickness'])
        mss[-1].add_par_linked_to_par('shale2Thickness', aress[-1].pars['shale2Thickness'])
        mss[-1].add_par_linked_to_par('shale3Thickness', aress[-1].pars['shale3Thickness'])
        mss[-1].add_par_linked_to_par('aquifer1Thickness', aress[-1].pars['aquifer1Thickness'])
        mss[-1].add_par_linked_to_par('aquifer2Thickness', aress[-1].pars['aquifer2Thickness'])
        mss[-1].add_par_linked_to_par('reservoirThickness', aress[-1].pars['reservoirThickness'])
        mss[-1].add_par_linked_to_par('brineResSaturation', aress[-1].pars['brineResSaturation'])
        mss[-1].add_par_linked_to_par('datumPressure', aress[-1].default_pars['datumPressure'])
    ...
    for ind in range(2):
        ...
        cas[-1].add_par_linked_to_par(
            'aqu_thick', aress[0].pars['aquifer{}Thickness'.format(ind+1)])
