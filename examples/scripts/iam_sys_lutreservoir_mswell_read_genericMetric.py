...
CO2saturation, and a metric called genericMetric from a .csv file. This example
is similar to the example iam_sys_lutr_mswell_read_temperature.py,
but it is intended to demonstrate that LookupTableReservoir can be used
to read any type of data (not just pressure, CO2saturation, and temperature).

This example uses a modified version of the file Reservoir_data_sim01.csv from the
Kimberlina data set (https://gitlab.com/NRAP/Kimberlina_data). The file should be located
...
to the times in columns 1 through 10 of the time_points.csv file in the same
folder). These columns were made to all have a constant value of 999.
...
should also be located in the Test_metrics folder and only has one entry
corresponding to the file Reservoir_data_sim01_read_genericMetric.csv.
The paramters are unchanged from the Kimberlina data set,
...
Although the genericMetric data are constant, this approach would also work
on metrics that vary over space and time.

Example of run:
$ python iam_sys_lutreservoir_mswell_read_genericMetric.py
...
if __name__ == '__main__':
    ...
    if not os.path.exists(os.sep.join([
            file_directory, 'Reservoir_data_sim01_read_genericMetric.csv'])):
    ...
    signature_data = np.genfromtxt(
        os.path.join(file_directory, 'parameters_and_filenames_read_genericMetric.csv'),
        delimiter=",", dtype='str')
    ...
    for ind in range(0, 1):
        ...
        intpr = sm.add_interpolator(ReservoirDataInterpolator(
            ...
            data_file='Reservoir_data_sim{ind1:02}_read_genericMetric.csv'.format(ind1=ind+1),
    ...
    ltres = sm.add_component_model_object(LookupTableReservoir(
        ...
        parameter_filename='parameters_and_filenames_read_genericMetric.csv'))

    # Add parameters of reservoir component model
    for j in range(num_pars):
        # Add arbitrary line (here, 1) of values from signature_file
        ltres.add_par(par_names[j], value=float(signature_data[1, j + 1]),
                      vary=False)

    # Add observations of reservoir component model
    ltres.add_obs('pressure')
    ltres.add_obs('CO2saturation')
    ltres.add_obs('genericMetric')
    ...
    genericMetric = sm.collect_observations_as_time_series(ltres, 'genericMetric')
    ...
    print('GenericMetric', genericMetric, sep='\n')
