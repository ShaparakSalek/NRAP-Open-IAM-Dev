...
AOR_CRIT_PRESSURE_COLUMNN = 'Critical Pressure [MPa]'

WORKFLOW_OPTIONS = ['AoR']

WORKFLOW_PLOT_NAMES = {
    'AoR': [{'Pressure_Plot': 'pressure'}, {'CO2_Saturation_Plot': 'CO2saturation'}, 
            {'Aq_CO2_Impact_Plot': '{AqCO2Metric}'}, {'Aq_Brine_Metric_Plot': '{AqSaltMetric}'}]
    }
...
AOR_AQUIFER_COMPONENT_OUTPUT = {'FutureGen2Aquifer': ['pH_volume', 'TDS_volume'], 
...
DEFAULT_LUTR_ENTRIES = {
    ...
    'TimeFile': 'time_points.csv', 'ParameterFilename': 'parameters_and_filenames_trunc.csv', 
    ...
    if 'Type' in yaml_data['Workflow']:
        if yaml_data['Workflow']['Type'] in WORKFLOW_OPTIONS:
            ...
            warning_msg = ''.join([
                ...
                '). The input in the Workflow section will therefore not be used.'])
            logging.warning(warning_msg)
    else:
        info_msg = ''.join([
            'A Workflow section was included in the .yaml file, but the section ', 
            'did not include a Type entry. The input in the Workflow section ', 
            'will therefore not be used.'])
        logging.info(info_msg)
    
    if not 'Options' in yaml_data['Workflow']:
        info_msg = ''.join([
...
def aor_workflow_setup(yaml_data, strata):
    ...
    if not 'Components' in yaml_data['ModelParams']:
    ...
    if not 'CriticalPressureMPa' in yaml_data['Workflow']['Options']:
...
def workflow_analysis(yaml_data, sm, analysis):
    ...
    if a Workflow section was included. This function then checks for and runs 
...
def get_points_in_aor(yaml_data, sm=None, time_index=None):
    ...
    AoR that relfects all metrics considered. Pressure results are only considered 
    ...
    if 'CriticalPressureMPa' in yaml_data['Workflow']['Options']:
        ...
        try:
            if var_type == 'noVariation':
                critPressureVal = np.min(AoR_pressure_results[AOR_CRIT_PRESSURE_COLUMNN])
            elif var_type == 'strikeAndDip' or var_type == 'LookupTable':
                critPressureVal = AoR_pressure_results[AOR_CRIT_PRESSURE_COLUMNN]
        except:
            if yaml_data['Workflow']['Options']['CriticalPressureMPa'] == 'Calculated':
                # returned in MPa
                critPressureVal = get_crit_pressure_aor_analysis(
                    pressure_x_km, pressure_y_km, yaml_data, sm)
            else:
        ...
        if var_type == 'noVariation':
            ...
            if len(pressure_x_km_AoR) > 0:
                for locRef in range(len(pressure_x_km_AoR)):
                    AoR_included_x_km.append(pressure_x_km_AoR[locRef])
                    AoR_included_y_km.append(pressure_y_km_AoR[locRef])
            
        elif var_type == 'strikeAndDip' or var_type == 'LookupTable':
            for locRef in range(len(pressure_x_km)):
                if pressure_vals_MPa[locRef] >= critPressureVal[locRef]:
                    AoR_included_x_km.append(pressure_x_km[locRef])
                    AoR_included_y_km.append(pressure_y_km[locRef])
    ...
    AoR_CO2saturation_results = pd.read_csv(os.path.join(IAM_DIR, output_dir, 
                                                         'csv_files', CO2saturation_file))
    ...
    if len(CO2saturation_x_km_AoR) > 0:
        for locRef in range(len(CO2saturation_x_km_AoR)):
            AoR_included_x_km.append(CO2saturation_x_km_AoR[locRef])
            AoR_included_y_km.append(CO2saturation_y_km_AoR[locRef])
    ...
    if len(CO2impact_x_km_AoR) > 0:
        for locRef in range(len(CO2impact_x_km_AoR)):
            AoR_included_x_km.append(CO2impact_x_km_AoR[locRef])
            AoR_included_y_km.append(CO2impact_y_km_AoR[locRef])
    ...
    if len(brineimpact_x_km_AoR) > 0:
        for locRef in range(len(brineimpact_x_km_AoR)):
            AoR_included_x_km.append(brineimpact_x_km_AoR[locRef])
            AoR_included_y_km.append(brineimpact_y_km_AoR[locRef])
    ...
    for locRef in range(len(All_x_points_km)):
        AoR_point_temp = (All_x_points_km[locRef], All_y_points_km[locRef])
        
        point_included = False
        for AoRLocRef in range(len(AoR_included_x_km)):
            AoR_included_loc_temp = (AoR_included_x_km[AoRLocRef], AoR_included_y_km[AoRLocRef])
            
            if AoR_point_temp == AoR_included_loc_temp:
                point_included = True
        
        if point_included:
            AoR_point_included[locRef] = 1
    ...
    with open(filename, 'w', newline='') as f:
        writer = csv.writer(f)
        for row_ref in range(0, len(All_x_points_km) + 1):
...
def remove_redundant_points(AoR_x_km, AoR_y_km):
    ...
    AoR_locs = []
    for AoRLocRef in range(len(AoR_x_km)):
        
        AoR_loc_temp = (AoR_x_km[AoRLocRef], AoR_y_km[AoRLocRef])
        
        if not AoR_loc_temp in AoR_locs:
            AoR_x_km_edit.append(AoR_x_km[AoRLocRef])
            AoR_y_km_edit.append(AoR_y_km[AoRLocRef])
...
def verify_AoR_points(pressure_x_km, pressure_y_km, CO2saturation_x_km, 
    ...
    """
    The x and y locations used in each of the plot types should be the same, but 
    but this function ensures that they are the same. If not, it produces an error.
    """
    x_all_same = True
    y_all_same = True
...
def add_aor_well_component_entries(yaml_data, well_component_type, aquifer_name, 
    ...
    output_list = AOR_WELLBORE_COMPONENT_OUTPUT
    
    for outputRef in range(len(output_list)):
        output_list[outputRef] = output_list[outputRef].format(aquiferNum)
...
def set_up_aor_plots_section(yaml_data, aquifer_component_type):
    ...
    for plotNum in range(len(plot_types_to_add)):
        plot = plot_types_to_add[plotNum]
        ...
        for plotName in list(plot.keys()):
            ...
            if 'TimeList' in yaml_data['Workflow']['Options']:
                if isinstance(yaml_data['Workflow']['Options']['TimeList'], list) or \
                    ...
                    warning_msg = ''.join(['The TimeList entry was provided under ', 
                                           ...
                                           'nor the word All. This input will ', 
            ...
            if 'PlotInjectionSites' in yaml_data['Workflow']['Options']:
                if isinstance(yaml_data['Workflow']['Options']['PlotInjectionSites'], bool):
                    ...
                    logging.warning(YAML_INPUT_WARNING_MSG.format(
            ...
            if not TimeList is None:
                yaml_data['Plots'][plotName]['TimeList'] = TimeList
            
            if not InjectionCoordx is None and not InjectionCoordy is None:
...
def get_automation_input(yaml_data):
    ...
    a Workflow.
    """
    automate_res_comp_setup = True
    automate_well_comp_setup = True
    automate_aq_comp_setup = True
    automate_plots_setup = True
    
    if 'AutomateResCompSetup' in yaml_data['Workflow']['Options']:
        if isinstance(yaml_data['Workflow']['Options']['AutomateResCompSetup'], bool):
            automate_res_comp_setup = yaml_data['Workflow']['Options']['AutomateResCompSetup']
        else:
            logging.warning(YAML_INPUT_WARNING_MSG.format(
                'AutomateResCompSetup', 'boolean', automate_res_comp_setup))
    
    if 'AutomateWellCompSetup' in yaml_data['Workflow']['Options']:
        if isinstance(yaml_data['Workflow']['Options']['AutomateWellCompSetup'], bool):
            automate_aq_comp_setup = yaml_data['Workflow']['Options']['AutomateWellCompSetup']
        else:
            logging.warning(YAML_INPUT_WARNING_MSG.format(
                'AutomateWellCompSetup', 'boolean', automate_well_comp_setup))
    
    if 'AutomateAqCompSetup' in yaml_data['Workflow']['Options']:
        if isinstance(yaml_data['Workflow']['Options']['AutomateAqCompSetup'], bool):
            automate_aq_comp_setup = yaml_data['Workflow']['Options']['AutomateAqCompSetup']
        else:
            logging.warning(YAML_INPUT_WARNING_MSG.format(
                'AutomateAqCompSetup', 'boolean', automate_aq_comp_setup))
    
    if 'AutomatePlotsSetup' in yaml_data['Workflow']['Options']:
        if isinstance(yaml_data['Workflow']['Options']['AutomatePlotsSetup'], bool):
            automate_plots_setup = yaml_data['Workflow']['Options']['AutomatePlotsSetup']
        else:
            logging.warning(YAML_INPUT_WARNING_MSG.format(
                'AutomatePlotsSetup', 'boolean', automate_plots_setup))
    
    automation_input = {'ResComp': automate_res_comp_setup, 
                        'WellComp': automate_well_comp_setup, 
                        'AqComp': automate_aq_comp_setup, 
                        'Plots': automate_plots_setup}
...
def get_crit_pressure_aor_analysis(pressure_x_km, pressure_y_km, yaml_data, sm):
    """
    ...
    if var_type == 'noVariation':
        ...
        critPressure = np.zeros((len(pressure_x_km), 1))
