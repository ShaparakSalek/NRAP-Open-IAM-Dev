...
def check_parameter_types(comp, par_name, run_again_info, sm=None, yaml_data=None):
        ...
        if par_val == 0:
            if 'Depth' in par_name and isinstance(comp, iam.Stratigraphy):
                ...
                if par_name == 'wellTop':
                    ...
                    if len(LeakTo) == 8:
                        run_again_info['unit_number'] = int(LeakTo[-1])
                    elif len(LeakTo) == 9:
                        run_again_info['unit_number'] = int(LeakTo[-2:None])
                    
                    run_again_info['top_or_bottom'] = 'bottom'
                    
                    run_again_info['par_name'] = LeakTo + 'Depth'
                    
                elif (par_name == 'reservoirDepth' or par_name == 'wellDepth'):
    ...
    elif par_name in comp.parlinked_pars and not sm is None:
