...
# Components where thre gridded output represents fault segments that each have 
...
# The formats for each metric type is currently the same, but using this dicitonary  
    ...
    for compRef, compType in enumerate(component_types):
        ...
        if compType in FAULT_COMPONENTS:
            ...
            if plot_val_over_area:
                if not GriddedMetric_yaml_input_dict['CellLengthX'] is None and \
                    not GriddedMetric_yaml_input_dict['CellLengthY'] is None:
        ...
        if no_results_check:
            ...
            for t_index in time_index_list:
                ...
                if comp_type_check:
                    metric = np.load(filename.format(
                ...
                for partRef in range(0, len(metric)):
                    cellMetric = metric[partRef]
                ...
                if not min_val_actual is None and not max_val is None:
                    ...
                    if min_val_actual == max_val:
                        title_str_edit = title_str + 'Value: {},\n{}(Gray: Results {})'.format(
                            max_val_str, metricSum_str, min_val_str)
                    else:
                        title_str_edit = title_str + 'Range: {} to {},\n{}(Gray: Results {})'.format(
                            min_val_actual_str, max_val_str, metricSum_str, min_val_str)
                else:
                    if min_value == 0:
                        min_val_str = '= 0' + unit_str
                    else:
...
def get_max_vals_over_time(components_to_use, num_times, metric_name, sim_index, 
    ...
    returns the minimum and maximum values. These values are used to set the 
    ...
    for compRef in range(0, len(components_to_use)):
        comp = components_to_use[compRef]
        comp_name = comp.name
        
        for t_index in range(0, num_times):
...
def get_comps_and_limits(components_name_list, yaml_data, name, sm):
    ...
    for comp in components:
            ...
            for compRef, compName in enumerate(components_name_list):
                if compName in comp.name:
                    ...
                    if comp.class_type in AREA_COMPONENTS:
                        ...
                        for cellRef in range(0, num_cells):
                        ...
                        component_xvals.append([])
                        component_yvals.append([])
                        for locRef in range(0, len(loc_data['coordx'])):
        ...
        if comp.class_type in RESERVOIR_COMPONENTS:
            ...
            GriddedMetric_yaml_input_dict = get_gridded_plot_yaml_input(yaml_data, name)
...
def update_min_max_xy(min_x_val, max_x_val, min_y_val, max_y_val, 
    ...
    x and y values.
    """
    for xRef in range(0, len(x_vals_to_check)):
        if x_vals_to_check[xRef] < min_x_val:
            min_x_val = x_vals_to_check[xRef]
        
        if x_vals_to_check[xRef] > max_x_val:
            max_x_val = x_vals_to_check[xRef]
    
    for yRef in range(0, len(y_vals_to_check)):
        if y_vals_to_check[yRef] < min_y_val:
            min_y_val = y_vals_to_check[yRef]
        
        if y_vals_to_check[yRef] > max_y_val:
            max_y_val = y_vals_to_check[yRef]
...
def get_gridded_plot_yaml_input(yaml_data, name, plot_type='GriddedMetric'):
    ...
    if cell_length_x is None and not cell_length_y is None:
        cell_length_y = None
        
    elif cell_length_y is None and not cell_length_x is None:
        cell_length_x = None
...
def get_t_indices(time_list, time_array):
    ...
    for time in time_list:
        ...
        for t_ref in range(0, len(time_array)):
            abs_diff[t_ref] = np.abs(time - (time_array[t_ref] / 365.25))
...
def plot_comp_inj_sites(ax, comp_name, comp_type, cell_xvals, cell_yvals, 
    ...
    if include_labels:
        ...
        plt.plot(np.array(cell_xvals) / 1000, np.array(cell_yvals) / 1000, 
                 ...
                 markersize=cellCenterMarkerSize, markerfacecolor='none', zorder=1e6, label=label)
    else:
        plt.plot(np.array(cell_xvals) / 1000, np.array(cell_yvals) / 1000, 
                 linestyle='none', marker='o', color='k', markeredgewidth=1.5, 
    ...
    if comp_type in FAULT_COMPONENTS and plot_faults:
        if not component_length is None and not component_strike is None \
            and not component_dip is None:
                for locRef in range(0, len(cell_xvals)):
                    center_x = cell_xvals[locRef]
                    center_y = cell_yvals[locRef]
                    ...
                    if locRef == 0 and include_labels:
                        plt.plot(np.array(fault_line_x) / 1000, np.array(fault_line_y) / 1000, 
...
def get_loc_from_strike(strike, input_distance):
    ...
    and a location a certain distance from that location in the direction of 
