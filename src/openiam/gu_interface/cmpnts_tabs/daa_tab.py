...
for DeepAlluviumAquifer component. Methods read and write dictionaries
...
# Set Deep Alluvium Aquifer parameters names and value, min, max, second value,
# mean, stddev, low_bound, upper_bound
DAA_PARAMETER_VALUES = {
    ...
    'correlationLengthX': [1098.235, 200, 2000, 750, 1000, 150, 200, 2000],
...
DAA_DYNAMIC_KWARGS = ['brine_rate', 'co2_rate', 'brine_mass', 'co2_mass']
...
def read_tab_vars(cmpnt_nm):
    ...
    cmpnt_data = commons_read_tab_vars(
        cmpnt_nm, 'DeepAlluviumAquifer', parameter_names=DAA_PARAMETERS,
        dynamic_kwarg_names=DAA_DYNAMIC_KWARGS,
        observation_names=DAA_OBSERVATIONS, add_aquifer=True)
...
def add_widgets(controller, tab, cmpnt_nm, cmpnt_type, toolTip,
    ...
    componentTypeDictionary.append('DeepAlluviumAquifer')
    ...
    for output_key in DAA_OBSERVATIONS:
    ...
    if 'Dynamic' in cnctn_nm:  # dynamic kwargs are provided instead of connected component
        for ind, key in enumerate(DAA_DYNAMIC_KWARGS):
            componentVars[cmpnt_nm][key] = dyn_data[ind]

    # Tab title label
    comp_type_label = ttk.Label(
        tab, text="Deep Alluvium Aquifer Component", font=LABEL_FONT)
    comp_type_label.grid(row=0, column=0, sticky='w', pady=(5, 10))

    # Parameters frames
    par_frames = {}
    for ind, par_name in enumerate(DAA_PARAMETERS):
        ...
        controller.setup_parameter_frame(
            ...
            {'lower_bound': DAA_PARAMETER_VALUES[par_name][6],
             'upper_bound': DAA_PARAMETER_VALUES[par_name][7]},
            DAA_PARAMETERS_SETUP[par_name][0],
            DAA_PARAMETERS_SETUP[par_name][1],
            PARAMETER_LABEL_WIDTH, DISTRIBUTION_MENU_WIDTH,
            DISTRIBUTION_ARG_LABEL_WIDTH, DISTRIBUTION_ARG_TEXTFIELD_WIDTH,
    ...
    outputs_frame.grid(row=19, column=0, sticky='w',
    ...
    for ind1 in range(4):
        for ind2 in range(3):
            ...
            output_nms_labels.append(
                ttk.Label(outputs_frame, text=DAA_OBSERVATIONS_SETUP[obs_nm][0],
            ...
            toolTip.bind(output_nms_checkboxes[-1],
                         DAA_OBSERVATIONS_SETUP[obs_nm][1])
