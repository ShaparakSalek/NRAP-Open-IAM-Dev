...
for CementedWellbore component. Methods read and write dictionaries
...
from openiam.gu_interface.cmpnts_tabs.locations import (read_locations_data,
                                                        add_wellbore_frame_widgets)
from openiam.gu_interface.cmpnts_tabs.commons import commons_read_tab_vars


CW_PARAMETERS = ['logWellPerm', 'logThiefPerm', 'wellRadius']
...
# Set Cemented Wellbore parameters names and value, min, max, second value, mean, std, bounds
CW_PARAMETER_VALUES = {'logWellPerm': [-13, -13.5, -10.2, -12, -12, 1, -13.95, -10.1],
                       'logThiefPerm': [-13, -13.9, -12.1, -13.5, -13, 0.5, -13.9991, -12.00035],
                       'wellRadius': [0.05, 0.025, 0.25, 0.1, 0.1, 0.04, 0.025, 0.25]}

CW_DYNAMIC_KWARGS = ['pressure', 'CO2saturation']

CW_OBSERVATIONS = ['CO2_aquifer1', 'brine_aquifer1', 'mass_CO2_aquifer1',
                   'CO2_aquifer2', 'brine_aquifer2', 'mass_CO2_aquifer2',
                   'CO2_atm', 'brine_atm']

CW_OBSERVATIONS_SETUP = {
    'CO2_aquifer1': [
...
def read_tab_vars(cmpnt_nm):
    ...
    cmpnt_data = commons_read_tab_vars(
        cmpnt_nm, 'CementedWellbore', parameter_names=CW_PARAMETERS,
        dynamic_kwarg_names=CW_DYNAMIC_KWARGS,
        observation_names=CW_OBSERVATIONS,
        add_number=True)
...
def add_widgets(controller, tab, cmpnt_nm, cmpnt_type, tool_tip,
    ...
    if 'Dynamic' in cnctn_nm:  # dynamic kwargs are provided instead of connected component
        for ind, key in enumerate(CW_DYNAMIC_KWARGS):
            componentVars[cmpnt_nm][key] = dyn_data[ind]

    # Populate dictionary
    componentVars[cmpnt_nm]['Params'] = controller.populate_params_dict(CW_PARAMETER_VALUES)

    for output_key in CW_OBSERVATIONS:
        componentVars[cmpnt_nm][output_key] = BooleanVar()
        componentVars[cmpnt_nm][output_key].set(0)

    # Tab title label
    comp_type_label = ttk.Label(
        tab, text="Cemented Wellbore Component", font=LABEL_FONT)
    comp_type_label.grid(row=0, column=0, sticky='w', pady=(5, 10))

    # Parameters frames
    par_frames = {}
    for ind, par_name in enumerate(CW_PARAMETERS):
        ...
        controller.setup_parameter_frame(
            ...
            {'lower_bound': CW_PARAMETER_VALUES[par_name][6],
             'upper_bound': CW_PARAMETER_VALUES[par_name][7]},
            CW_PARAMETERS_SETUP[par_name][0],
            CW_PARAMETERS_SETUP[par_name][1],
            PARAMETER_LABEL_WIDTH, DISTRIBUTION_MENU_WIDTH,
            DISTRIBUTION_ARG_LABEL_WIDTH, DISTRIBUTION_ARG_TEXTFIELD_WIDTH,
    ...
    for ind1 in range(3):
        for ind2 in range(ind2_range[ind1]):
            ...
            output_nms_labels.append(
                ttk.Label(outputs_frame, text=CW_OBSERVATIONS_SETUP[obs_nm][0],
            ...
            tool_tip.bind(output_nms_checkboxes[-1],
                          CW_OBSERVATIONS_SETUP[obs_nm][1])
