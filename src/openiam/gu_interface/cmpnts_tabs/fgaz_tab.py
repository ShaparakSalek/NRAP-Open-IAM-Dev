...
for FutureGen2AZMI component. Methods read and write dictionaries
...
from openiam.gu_interface.dictionarydata import DISTRIBUTION_OPTIONS, connections
...
FGAZ_PARAMETERS = ['por', 'log_permh', 'log_aniso', 'rel_vol_frac_calcite']

FGAZ_PARAMETERS_SETUP = {
...
# Set FutureGen 2 AZMI parameters names and value, min, max, second value, mean,
# std, low_bound, upper_bound
FGAZ_PARAMETER_VALUES = {
...
FGAZ_DYNAMIC_KWARGS = ['brine_rate', 'co2_rate', 'brine_mass', 'co2_mass']
...
FGAZ_OBSERVATIONS = [
    ...
    'Temperature_volume', 'Temperature_dx', 'Temperature_dy', 'Temperature_dz']

FGAZ_OBSERVATIONS_SETUP = {
    ...
    'Dissolved_CO2_dz': [
        ...
        ''.join(['Enable height z of aquifer volume above baseline ',
                 'dissolved CO{} as output.']).format(u'\u2082')],
    'Temperature_volume': [
        "Volume above baseline {}T [m{}]".format(u'\u0394', u'\u00B3'),
        'Enable aquifer volume above baseline temperature change as output.'],
    'Temperature_dx': [
        "Length x of volume above baseline {}T [m]".format(u'\u0394'),
        ''.join(['Enable length x of aquifer volume above baseline ',
                 'temperature change as output.'])],
    'Temperature_dy': [
        "Width y of volume above baseline {}T [m]".format(u'\u0394'),
        'Enable width y of aquifer volume above baseline temperature change as output.'],
    'Temperature_dz': [
        "Height z of volume above baseline {}T [m]".format(u'\u0394'),
        'Enable height z of aquifer volume above baseline temperature change as output.']}
...
def read_tab_vars(cmpnt_nm):
    ...
    cmpnt_data = commons_read_tab_vars(
        cmpnt_nm, 'FutureGen2AZMI', parameter_names=FGAZ_PARAMETERS,
        dynamic_kwarg_names=FGAZ_DYNAMIC_KWARGS,
        observation_names=FGAZ_OBSERVATIONS, add_aquifer=True)
...
def add_widgets(controller, tab, cmpnt_nm, cmpnt_type, tool_tip,
    ...
    componentTypeDictionary.append('FutureGen2AZMI')
    ...
    for output_key in FGAZ_OBSERVATIONS:
    ...
    if 'Dynamic' in cnctn_nm:  # dynamic kwargs are provided instead of connected component
        for ind, key in enumerate(FGAZ_DYNAMIC_KWARGS):
            componentVars[cmpnt_nm][key] = dyn_data[ind]

    # Tab title label
    comp_type_label = ttk.Label(
        tab, text="FutureGen 2 AZMI Component", font=LABEL_FONT)
    comp_type_label.grid(row=0, column=0, sticky='w', pady=(5, 10))

    # Parameters frames
    par_frames = {}
    for ind, par_name in enumerate(FGAZ_PARAMETERS):
        ...
        controller.setup_parameter_frame(
            ...
            {'lower_bound': FGAZ_PARAMETER_VALUES[par_name][6],
             'upper_bound': FGAZ_PARAMETER_VALUES[par_name][7]},
            FGAZ_PARAMETERS_SETUP[par_name][0],
            FGAZ_PARAMETERS_SETUP[par_name][1],
            PARAMETER_LABEL_WIDTH, DISTRIBUTION_MENU_WIDTH,
            DISTRIBUTION_ARG_LABEL_WIDTH, DISTRIBUTION_ARG_TEXTFIELD_WIDTH,
    ...
    # Auxiliary variable needed to create different number of rows
    # in each column of outputs
    num_rows = [8, 8, 4]

    for ind1 in range(3):
        for ind2 in range(num_rows[ind1]):
            ind = ind1*8 + ind2
            obs_nm = FGAZ_OBSERVATIONS[ind]
            ...
            output_nms_labels.append(
                ttk.Label(outputs_frame, text=FGAZ_OBSERVATIONS_SETUP[obs_nm][0],
            ...
            tool_tip.bind(output_nms_checkboxes[-1],
                          FGAZ_OBSERVATIONS_SETUP[obs_nm][1])
