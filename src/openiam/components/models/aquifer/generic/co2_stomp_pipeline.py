...
# convert CMG GEM simulaton data into images
    ...
    def load(self):
        ...
        self.input_images = np.stack((
            xcoord_array, zcoord_array, xperm_array, zperm_array,
            por_array, initsalt_array, co2mass_array, watermass_array,
            saltmass_array, time_array), axis=4)
        ...
        self.input_mins = [
            xcoord_array.min(), zcoord_array.min(),
        ...
        self.input_maxs = [
            xcoord_array.max(), zcoord_array.max(),
    ...
    def resize(self):
        ...
        def convert_image(input_image, output_image):
            ...
            # input_image, output_image = resize_image(
            #     input_image, output_image, self.image_height, self.image_width)
            return input_image, output_image
...
if __name__ == '__main__':
    ...
    # train_dataset = tf.data.Dataset.from_tensor_slices((train.input_trans, train.output_trans))
    # test_dataset = tf.data.Dataset.from_tensor_slices((test.input_resized, test.output_resized))
